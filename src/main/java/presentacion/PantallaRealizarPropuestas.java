	...
	public PantallaRealizarPropuestas() {
				...
				try {
					...
					if (!compararFechas(fechaInicio, fechaFinal, currentDate)) {
						...
						if (!existeEdicion) {
							...
							JOptionPane.showMessageDialog(null, "La edici�n ya existe en la base de datos.", error,
									JOptionPane.ERROR_MESSAGE);
		...
		btnSiguiente.addActionListener(event -> {
			...
			if (hayCamposVacios()) {
				...
				JOptionPane.showMessageDialog(null, "Introduzca el DNI del secretario con todos sus d�gitos.", error,
						JOptionPane.ERROR_MESSAGE);
				...
				JOptionPane.showMessageDialog(null, "Debe seleccionar una facultad.", error, JOptionPane.ERROR_MESSAGE);
			} else {
